...
export function AttackTargetPicker({
  ...
  const commonTargets = useMemo(
    ...
    [selectedAttackers, sortedTargets, validTargetsByAttacker, validTargets],
  );
  ...
  function incOnTarget(stack: AttackerStack, target: AttackTarget) {
    mutate((next) => {
      const id = lowestUnassignedForTarget(stack, target, next, targetsForCreature);
  ...
  function allOfStackToTarget(stack: AttackerStack, target: AttackTarget) {
    if (!stackFullySupportsTarget(stack, target, targetsForCreature)) return;
    mutate((next) => {
  ...
  function spreadStack(stack: AttackerStack) {
    mutate((next) => spreadStackEvenly(next, stack, sortedTargets, targetsForCreature));
  }

  function spreadAll() {
    mutate((next) => {
      for (const stack of stacks) {
        spreadStackEvenly(next, stack, sortedTargets, targetsForCreature);
      }
    });
  }

  function allStacksToTarget(target: AttackTarget) {
    if (!commonTargets.some((candidate) => sameAttackTarget(candidate, target))) return;
    mutate((next) => {
  ...
  function countOnTarget(stack: AttackerStack, target: AttackTarget): number {
    ...
    return stack.ids.reduce((n, id) => {
      ...
      return n + (assigned && attackTargetKey(assigned) === key ? 1 : 0);
    }, 0);
  }
  ...
  const slideTransform = peeked ? { x: "calc(100vw - 32px)" } : { x: 0 };
      ...
      >
        ...
        >
          <div className="flex max-h-[85vh] flex-col overflow-hidden rounded-xl border border-gray-600 bg-gray-900/95 shadow-2xl backdrop-blur-sm">
            ...
            <div
              className={`min-h-0 flex-1 overflow-y-auto overflow-x-hidden overscroll-contain thin-scrollbar pb-2 pt-4 ${sidePadding}`}
            >
              {mode === "all" ? (
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p
                      className={`text-xs font-medium ${unassignedTotal > 0 ? "text-amber-300" : "text-emerald-300"}`}
                    >
                      {unassignedTotal > 0
                        ? t("attackTargetPicker.unassignedRemaining", {
                    ...
                    <table className="w-full border-separate border-spacing-0 text-sm">
                      <thead>
                        <tr>
                          ...
                          {sortedTargets.map((target) => {
                            const color = getTargetSeatColor(target);
                            const canAssignAll = commonTargets.some((candidate) =>
                              ...
                              >
                                <div className="flex flex-col items-center gap-1">
                                  ...
                                  <button
                                    ...
                                    className="rounded border border-gray-600 px-1.5 py-0.5 text-[10px] font-medium text-gray-300 hover:border-gray-400 hover:bg-white/10 disabled:cursor-default disabled:opacity-30"
                                  >
                      ...
                      <tbody>
                        {stacks.map((stack) => {
                          ...
                          const canSpread = stack.ids.some(
                            (id) => targetsForCreature(id).length > 0,
                          );
                          return (
                            <tr key={stack.key} className="border-t border-white/5">
                              ...
                              {sortedTargets.map((target) => {
                                ...
                                const canInc =
                                  lowestUnassignedForTarget(
                                    stack,
                                    target,
                                    assignments,
                                    targetsForCreature,
                                  ) != null;
                                const canAll = stackFullySupportsTarget(
                                  stack,
                                  target,
                                  targetsForCreature,
                                );
                                return (
                                  ...
                                  >
                                    <StepperCell
                                      ...
                                      decTitle={t("attackTargetPicker.removeOne", {
                                        label,
                                      })}
                                      incTitle={t("attackTargetPicker.assignOne", {
                                        label,
                                      })}
                                      allTitle={t("attackTargetPicker.assignAllHere", {
                                        label,
                                      })}
                                    />
                                  </td>
                      ...
                      return (
                        ...
                        >
                          <button
                            type="button"
                            onClick={() =>
                              setExpandedStack((cur) =>
                                cur === stack.key ? null : stack.key,
                              )
                            }
                          ...
                          {expanded && (
                            <div className="flex flex-col gap-1.5 border-t border-white/10 px-2 py-2">
                              <button
                                ...
                                onClick={() => spreadStack(stack)}
                                disabled={!canSpread}
                                className={`self-start ${gameButtonClass({
                                  tone: "indigo",
                                  size: "xs",
                                  disabled: !canSpread,
                                })}`}
            ...
            <div className={`shrink-0 border-t border-white/10 pb-5 pt-3 ${sidePadding}`}>
              {mode === "distribute" && (
                <button
                  ...
                  className={`w-full ${gameButtonClass({
                    tone: "emerald",
                    size: "md",
                    disabled: unassignedTotal > 0,
                  })}`}
                >
                  {unassignedTotal > 0
                    ? t("attackTargetPicker.assignRemaining", {
                        count: unassignedTotal,
                      })
                    : t("attackTargetPicker.confirmDistribute", {
                        count: selectedAttackers.length,
                      })}
                </button>
              )}
              <button
                onClick={onCancel}
                className={`w-full ${mode === "distribute" ? "mt-2" : ""} ${gameButtonClass({
                  tone: "slate",
                  size: "sm",
                })}`}
              >
  ...
  return Object.entries(obj.counters)
    .filter(
      (entry): entry is [string, number] =>
        entry[1] != null && entry[1] > 0 && entry[0] !== "loyalty",
    )
...
function lowestUnassignedForTarget(
  stack: AttackerStack,
  target: AttackTarget,
  map: AssignmentMap,
...
function highestOnTarget(
  ...
  for (let i = stack.ids.length - 1; i >= 0; i--) {
    const assigned = map.get(stack.ids[i]);
    if (assigned && attackTargetKey(assigned) === key) {
      return stack.ids[i];
    }
  }
  return null;
...
function stackFullySupportsTarget(
  stack: AttackerStack,
  target: AttackTarget,
  targetsForCreature: (creatureId: ObjectId) => AttackTarget[],
): boolean {
  return stack.ids.every((id) =>
    targetsForCreature(id).some((candidate) => sameAttackTarget(candidate, target)),
  );
}
  ...
  return (
    <div className="flex items-center justify-center gap-1">
      ...
      <button
        ...
        title={allTitle}
        aria-label={allTitle}
        className={`min-w-[2.75rem] rounded px-1 py-2.5 text-center text-sm font-semibold tabular-nums hover:bg-white/10 disabled:cursor-default disabled:opacity-30 md:min-w-[1.9rem] md:py-0.5 ${count > 0 ? "text-gray-100" : "text-gray-500"}`}
...
function getPlayerLabel(
  ...
  if (teamBased && Math.floor(playerId / 2) === Math.floor(myId / 2)) {
    return t("attackTargetPicker.ally");
  }
